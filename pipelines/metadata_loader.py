...
def load_metadata():
    ...
    if not os.path.isdir(root):
        raise FileNotFoundError(
            f"The directory specified in DATA_ROOT does not exist: {root}"
        )
    ...
    for ship_class in os.listdir(root):
        ...
        meta = [
            f for f in os.listdir(os.path.join(root, ship_class)) if identifier in f
        ]
        ...
        if meta:
            ...
            df = pd.read_csv(
                os.path.join(root, ship_class, meta[0]),
                names=[
                    "class-id",
                    "ship-name",
                    "day",
